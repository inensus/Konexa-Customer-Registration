...
namespace Inensus\KonexaBulkRegistration\Test\Feature;
...
class ImportCsv extends TestCase
{
    ...
    public function is_user_sent_csv()
    {
        ...
        $response = $this->actingAs($user)->post('/api/konexa-bulk-register/import-csv', $csv_data_file);
        $createdCsvData = KonexaCsvData::query()->first();
        $jsonResponse = json_decode($response->getContent(), true);
        $response->assertStatus(201);
        $response->assertJson([
            'data' => [
                'type' => 'konexa_csv_data',
                'konexa_csv_data_id' => $createdCsvData->id,
                'attributes' => [
